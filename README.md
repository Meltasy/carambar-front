# carambar-front

[![License ISC](https://img.shields.io/github/license/Meltasy/blog-api)](https://opensource.org/license/isc-license-txt)
[![ECMAScript](https://img.shields.io/badge/ECMAScript-2025-blue.svg)](https://ecma-international.org/publications-and-standards/standards/ecma-262/)
[![Repo Size](https://img.shields.io/github/repo-size/Meltasy/blog-api)](https://github.com/Meltasy/carambar-front)

Frontend pour une mini application web de blagues.

Découvrez [Carambar Blagues](https://meltasy.github.io/carambar-front/) !

## Tech Stack

* HTML et CSS
* Vanilla JavaScript (ES6+)

## License

L'ensemble de ce projet est sous licence MIT. Pour plus d'informations, consultez le fichier [LICENSE](LICENSE).